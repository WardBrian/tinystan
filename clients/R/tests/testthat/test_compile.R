...
test_that("compilation fails on non-stan file", {
  expect_error(
    compile_model(file.path(stan_folder, "bernoulli", "bernoulli.data.json")),
...
test_that("compilation fails on bad syntax", {
  expect_error(
    compile_model(file.path(stan_folder, "syntax_error", "syntax_error.stan")),
    "Compilation failed"
  )
})
...
test_that("bad paths fail", {
  ...
  expect_error(
    set_tinystan_path(file.path(stan_folder)),
