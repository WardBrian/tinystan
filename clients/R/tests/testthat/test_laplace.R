...
test_that("bad data handled properly", {
  data <- "{\"N\": -1}"
  expect_error(
    laplace_sampler(bernoulli_model, BERNOULLI_MODE, data),
  ...
  expect_error(
    laplace_sampler(bernoulli_model, BERNOULLI_MODE, "{\"bad\"}"),
    "Error in JSON parsing"
  )

  expect_error(
    laplace_sampler(bernoulli_model, BERNOULLI_MODE, "not/real/path.json"),
...
test_that("bad mode array handled properly", {
  mode1 = c(2)
  expect_error(
    laplace_sampler(bernoulli_model, mode1, BERNOULLI_DATA),
    "Bounded variable is 2"
  )

  mode2 = c(1.0)
  expect_error(
    laplace_sampler(gaussian_model, mode2, "{\"N\": 4 }"),
    "incorrect length"
  )
})
...
test_that("bad mode json handled properly", {
  mode <- "{\"theta\": 2}"
  expect_error(
    laplace_sampler(bernoulli_model, mode, BERNOULLI_DATA),
    "Bounded variable is 2"
  )
  ...
  expect_error(
    laplace_sampler(bernoulli_model, "bad/path.json", BERNOULLI_DATA),
    "Could not open data file"
  )
})
