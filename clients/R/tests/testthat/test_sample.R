...
test_that("seed works", {
    ...
    out1 <- sampler(bernoulli_model, BERNOULLI_DATA, seed = 123, num_warmup = 100,
        num_samples = 100)
...
test_that("bad data handled properly", {
    ...
    expect_error(sampler(bernoulli_model, "not/real/path.json"), "Could not open data file")
...
test_that("bad inits handled properly", {
    ...
    init1 <- "{\"theta\": 2}"
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, inits = init1), "Initialization failed")

    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, inits = "bad/path.json"),
        "Could not open data file")

    init2 <- "{\"theta\": 0.5}"
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_chains = 2, inits = c(init2,
        init1)), "Initialization failed")

    inits <- list(init2, init2)
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_chains = 1, inits = inits),
        "match the number of chains")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_chains = 3, inits = inits),
        "match the number of chains")
...
test_that("bad initial metric shape handled properly", {
    ...
    expect_error(sampler(gaussian_model, data, metric = tinystan::HMCMetric$DENSE,
        init_inv_metric = rep(1, 5)), "Invalid initial metric size")

    expect_error(sampler(gaussian_model, data, metric = tinystan::HMCMetric$DENSE,
        init_inv_metric = matrix(1, 5, 4)), "Invalid initial metric size")

    expect_error(sampler(gaussian_model, data, num_chains = 4, metric = tinystan::HMCMetric$DENSE,
        init_inv_metric = array(1, c(5, 5, 3))), "Invalid initial metric size")

    expect_error(sampler(gaussian_model, data, metric = tinystan::HMCMetric$DIAGONAL,
        init_inv_metric = rep(1, 4)), "Invalid initial metric size")

    expect_error(sampler(gaussian_model, data, num_chains = 4, metric = tinystan::HMCMetric$DIAGONAL,
...
test_that("bad initial metric handled properly", {
    ...
    metric[, , 1] <- 1e+20
    metric[, , 2] <- diag(3)
    expect_error(sampler(gaussian_model, data, num_chains = 2, metric = tinystan::HMCMetric$DENSE,
        init_inv_metric = metric), "not positive definite")

})
test_that("bad num_warmup handled properly", {

    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_warmup = -1), "non-negative")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, save_warmup = TRUE, num_warmup = -1),
        "non-negative")
...
test_that("bad args raise errors", {
    ...
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_chains = 0), "at least 1")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_samples = 0), "at least 1")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, id = 0), "positive")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, init_radius = -0.1), "non-negative")

    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, delta = -0.1), "between 0 and 1")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, delta = 1.1), "between 0 and 1")
    ...
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, stepsize = 0), "positive")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, stepsize_jitter = -0.1),
        "between 0 and 1")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, stepsize_jitter = 1.1),
        "between 0 and 1")

    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, max_depth = 0), "positive")
    expect_error(sampler(bernoulli_model, BERNOULLI_DATA, num_threads = 0), "positive")
