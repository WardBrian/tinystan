...
test_that("bad data handled properly", {
  ...
  expect_error(
    optimizer(bernoulli_model, "not/real/path.json"),
...
test_that("bad init handled properly", {
  init <- "{\"theta\": 2}"
  expect_error(
    optimizer(bernoulli_model, BERNOULLI_DATA, init = init),
    "Initialization failed"
  )

  expect_error(
    optimizer(bernoulli_model, BERNOULLI_DATA, init = "bad/path.json"),
    "Could not open data file"
  )
})
...
test_that("bad args raise errors", {
  for (algorithm in ALGORITHMS) {
    expect_error(
      optimizer(bernoulli_model, BERNOULLI_DATA, algorithm = algorithm, id = 0),
      "positive"
    )
    expect_error(
      optimizer(
