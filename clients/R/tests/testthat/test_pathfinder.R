...
test_that("bad inits handled properly", {
  ...
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_paths=1, inits=init), "Initialization failed")

  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, inits="bad/path.json"), "Could not open data file")

  inits <- c(init, init)
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_paths=2, inits=inits), "Initialization failed")
  ...
  inits[[11]] <- init2
  bernoulli_model$pathfinder(BERNOULLI_DATA, num_paths=11, num_multi_draws=10, inits=inits)

  inits <- list(init2, init2)
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_paths=1, inits=inits), "match the number of chains")
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_paths=3, inits=inits), "match the number of chains")
...
test_that("bad args raise errors", {
  ...
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, init_radius=-0.1), "non-negative")
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_threads=0), "positive")
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_iterations=0), "positive")
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_elbo_draws=0), "positive")
  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, num_multi_draws=0), "at least 1")

  expect_error(bernoulli_model$pathfinder(BERNOULLI_DATA, max_history_size=0), "positive")
