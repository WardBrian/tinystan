...
test_that("seed works", {
  ...
  expect_equal(
    sort(posterior::draws_of(out1$draws$theta)),
    sort(posterior::draws_of(out2$draws$theta))
  )

  out3 <- pathfinder(bernoulli_model, BERNOULLI_DATA, seed = 456)
  expect_error(expect_equal(
    sort(posterior::draws_of(out1$draws$theta)),
    sort(posterior::draws_of(out3$draws$theta))
  ))
})
...
test_that("bad data handled properly", {
  ...
  expect_error(
    pathfinder(bernoulli_model, "{\"bad\"}"),
    "Error in JSON parsing"
  )

  expect_error(
    pathfinder(bernoulli_model, "not/real/path.json"),
...
test_that("bad inits handled properly", {
  init <- "{\"theta\": 2}"
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, inits = init),
    "Initialization failed"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_paths = 1, inits = init),
    "Initialization failed"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, inits = "bad/path.json"),
    "Could not open data file"
  )
  inits <- c(init, init)
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_paths = 2, inits = inits),
    "Initialization failed"
  )
  ...
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_paths = 1, inits = inits),
    "match the number of chains"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_paths = 3, inits = inits),
    "match the number of chains"
  )
})
...
test_that("bad args raise errors", {
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_paths = 0),
    "at least 1"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_draws = 0),
    "at least 1"
  )
  expect_error(pathfinder(bernoulli_model, BERNOULLI_DATA, id = 0), "positive")
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, init_radius = -0.1),
    "non-negative"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_threads = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_iterations = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_elbo_draws = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, num_multi_draws = 0),
    "at least 1"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, max_history_size = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, init_alpha = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, tol_obj = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, tol_rel_obj = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, tol_grad = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, tol_rel_grad = 0),
    "positive"
  )
  expect_error(
    pathfinder(bernoulli_model, BERNOULLI_DATA, tol_param = 0),
    "positive"
  )
})
