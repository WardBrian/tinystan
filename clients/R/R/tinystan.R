...
with_model = function(model, data, seed, block) {
    ...
    handle_error(all(ffi_ret$model == 0), model$lib_name, ffi_ret$err)
    tryCatch({
        # this is the equivalent of base R's `with` function
        eval(substitute(block), ffi_ret, enclos = parent.frame())
    }, finally = {
        .C("tinystan_destroy_model_R", as.raw(ffi_ret$model), PACKAGE = model$lib_name)
    })
}
...
#' @noRd
get_parameter_names.tinystan_model = function(model, model) {
...
#' @noRd
get_free_params.tinystan_model = function(model, model) {
...
sampler.tinystan_model = function(model, data = "", num_chains = 4, inits = NULL,
    ...
    with_model(model, data, seed, {
        free_params <- get_free_params(model, model)
        if (free_params == 0) {
            stop("Model has no parameters to sample")
        }

        params <- c(HMC_SAMPLER_VARIABLES, get_parameter_names(model, model))
...
#' @export
pathfinder.tinystan_model = function(model, data = "", num_paths = 4, inits = NULL, seed = NULL,
    ...
    with_model(model, data, seed, {
        ...
        params <- c(PATHFINDER_VARIABLES, get_parameter_names(model, model))
        num_params <- length(params)
...
#' @export
optimizer.tinystan_model = function(model, data = "", init = NULL, seed = NULL, id = 1, init_radius = 2,
    ...
    with_model(model, data, seed, {
        params <- c(OPTIMIZATION_VARIABLES, get_parameter_names(model, model))
        num_params <- length(params)
...
#' @export
laplace_sampler.tinystan_model = function(model, mode, data = "", num_draws = 1000, jacobian = TRUE,
    ...
    with_model(model, data, seed, {
        params <- c(LAPLACE_VARIABLES, get_parameter_names(model, model))
        num_params <- length(params)
        free_params <- get_free_params(model, model)
