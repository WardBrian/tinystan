...
tinystan_model = function(lib, stanc_args = NULL, make_args = NULL, warn = TRUE) {
  ...
  sep <- rawToChar(sep)
  ret <- list(lib = lib, lib_name = lib_name, sep = sep)
...
print.tinystan_model <- function(mod, ...) {
  cat("Stan model\n")
  cat("  Library: ", mod$lib, "\n")
  cat("  Library name: ", mod$lib_name, "\n")
  cat("  Separator: ", mod$sep, "\n")
}
...
#' @noRd
with_model <- function(...) {
  UseMethod("with_model")
}

#' @noRd
with_model.tinystan_model = function(private, data, seed, block) {
